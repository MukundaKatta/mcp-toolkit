...
export class Server extends EventEmitter {
  ...
  constructor(options: ServerOptions = {}) {
    super();
    this.options = { name: options.name ?? "Server", enabled: options.enabled ?? true, timeout: options.timeout ?? 30000 };
  }
  ...
  async process(input: unknown): Promise<ServerResult> {
