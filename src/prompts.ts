...
export class Prompts extends EventEmitter {
  private readonly options: Required<PromptsOptions>;
  private initialized = false;

  constructor(options: PromptsOptions = {}) {
    super();
    this.options = { name: options.name ?? "Prompts", enabled: options.enabled ?? true, timeout: options.timeout ?? 30000 };
  }
  ...
  async process(input: unknown): Promise<PromptsResult> {
