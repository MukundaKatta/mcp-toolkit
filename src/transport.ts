...
export class Transport extends EventEmitter {
  ...
  constructor(options: TransportOptions = {}) {
    super();
    this.options = { name: options.name ?? "Transport", enabled: options.enabled ?? true, timeout: options.timeout ?? 30000 };
  }
  ...
  async process(input: unknown): Promise<TransportResult> {
